...
> `ui-rules/` or `patterns/` must trace its source here (via `references/research-log.md`).
...
5. This table updates in Phase 0 if a reliable skill appears for Fluent/Carbon/Primer
   (check on skills.sh, then test), otherwise never without an explicit request.

## History

- 2026-08-06: initial verification (details in `references/research-log.md`).
