# Consumption guide — ui-dev-kit

> How to use `ui-sdk/` in a real Wails app (or any React + Vite + TS project). This is the
> contract every consumer must follow. Verified in Phase 3 (2026-08-06).
...
in `references/wails-constraints.md`. The contract:
...
1. **Static build only.** The frontend must build to a static bundle at `frontend/dist/`
   ...
   Every piece in this kit already complies (Next-specific pieces were excluded in Phase 2).
