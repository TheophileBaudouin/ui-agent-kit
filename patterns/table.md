...
- `ui-sdk/blocks/blocks-so/stats-12.tsx` (usage table dashboard) — not a table block per se;
  see `references/phase-2-report.md` for `table-05` (excluded — built against a different
  Table API; re-integrate in Phase 3+ if the newer `@tanstack/react-table` is adopted).
